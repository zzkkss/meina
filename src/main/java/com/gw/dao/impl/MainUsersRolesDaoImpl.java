...
@Repository("mainUsersRolesDao")
public class MainUsersRolesDaoImpl extends BaseDaoImpl<MainUsersRoles> implements MainUsersRolesDao {


	public Map<String, Object> getByPage(Jqpage jqpage, String... strings) {
		// TODO Auto-generated method stub
		Criteria criteria=this.getSession().createCriteria(MainUsersRoles.class);
		...
		jqpage.setTotalRecords(criteria.list().size());
			...
			List<MainUsersRoles> list= criteria.list();
	...
	public List<MainUsersRoles> lastUser(int num) {
		...
		List<MainUsersRoles> a=criteria.list();
	return a;
	}
...
//		criteria.add(Expression.eq("Type", strings[0]));
	...
	public List<MainUsersRoles> findByUsers(MainUsers authority) {
		...
		Criteria criteria=this.getSession().createCriteria(MainUsersRoles.class);
//		criteria.add(Expression.eq("Type", strings[0]));
		criteria.add(Restrictions.eq("UsersId", authority.getId()));
		criteria.addOrder(Order.desc("id"));
		List<MainUsersRoles> a=criteria.list();
	return a;
	}
