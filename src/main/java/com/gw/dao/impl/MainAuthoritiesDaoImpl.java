...
@Repository("mainAuthoritiesDao")
public class MainAuthoritiesDaoImpl extends BaseDaoImpl<MainAuthorities> implements MainAuthoritiesDao {


	public Map<String, Object> getByPage(Jqpage jqpage, String... strings) {
		// TODO Auto-generated method stub
		Criteria criteria=this.getSession().createCriteria(MainAuthorities.class);
		...
		jqpage.setTotalRecords(criteria.list().size());
			...
			List<MainAuthorities> list= criteria.list();
	...
	public List<MainAuthorities> lastUser(int num) {
		...
		List<MainAuthorities> a=criteria.list();
	return a;
	}

	public List<MainAuthorities> findByRoles(MainRolesAuthorities mainRolesAuthorities2) {
		// TODO Auto-generated method stub
		Criteria criteria=this.getSession().createCriteria(MainAuthorities.class);
//		criteria.add(Expression.eq("Type", strings[0]));
		criteria.add(Restrictions.eq("AuthoritiesId", mainRolesAuthorities2.getAuthoritiesId()));
		criteria.addOrder(Order.desc("id"));
		List<MainAuthorities> a=criteria.list();
	return a;
	}
	...
	public MainAuthorities findByMainAuthoritiesResources(
		...
		Criteria criteria=this.getSession().createCriteria(MainAuthorities.class);
//		criteria.add(Expression.eq("Type", strings[0]));
		criteria.add(Restrictions.eq("Id", mar.getAuthoritiesId()));
		criteria.addOrder(Order.desc("Id"));
		List<MainAuthorities> a=criteria.list();
		return a.get(0);
	}
	...
	public List<MainAuthorities> findByMainRolesAuthorities(
			MainRolesAuthorities m) {
		// TODO Auto-generated method stub
		Criteria criteria=this.getSession().createCriteria(MainAuthorities.class);
//		criteria.add(Expression.eq("Type", strings[0]));
		criteria.add(Restrictions.eq("Id", m.getAuthoritiesId()));
		criteria.addOrder(Order.desc("Id"));
		List<MainAuthorities> a=criteria.list();
		return a;
	}
