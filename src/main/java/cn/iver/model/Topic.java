...
public class Topic extends Model<Topic>{
    ...
    private static final String TOPIC_PAGE_FOR_MY_CACHE = "topicPageForMy";
    ...
    public Page<Topic> getPage(int pageNumber){
        Page<Topic> topicPage = dao.paginateByCache(TOPIC_PAGE_FOR_INDEX_CACHE, pageNumber,
                ...
                "select id", "from bbs_topic order by createTime desc");
    ...
    public Page<Topic> getPageForModule(int moduleID, int pageNumber){
        Page<Topic> topicPage = dao.paginateByCache(TOPIC_PAGE_FOR_MODULE_CACHE, moduleID + "-" + pageNumber,
                pageNumber, Const.TOPIC_PAGE_SIZE,
                "select id", "from bbs_topic where moduleID=? order by createTime desc", moduleID);
        return loadModelPage(topicPage);
    }
    public Page<Topic> getHotPage(int pageNumber){
        ...
        Page<Topic> topicPage = dao.paginateByCache(cacheName, pageNumber,
                pageNumber, Const.TOPIC_PAGE_SIZE,
                "select id", "from bbs_topic order by pv desc");
        return loadModelPage(topicPage);
    }
    public Page<Topic> getNicePage(int pageNumber){
        ...
        Page<Topic> topicPage = dao.paginateByCache(cacheName, pageNumber,
                pageNumber, Const.TOPIC_PAGE_SIZE,
                "select id", "from bbs_topic where isNice=true order by createTime desc");
        return loadModelPage(topicPage);
    }
    ...
    public Page<Topic> getPageForMy(Integer pageNumber, String userID) {
        String cacheName = TOPIC_PAGE_FOR_MY_CACHE;
        Page<Topic> topicPage = dao.paginateByCache(cacheName, pageNumber,
                pageNumber, Const.PAGE_SIZE_FOR_MY,
                "select id", "from bbs_topic where userid =? order by createTime desc",userID);
        System.out.println(userID);
        return loadModelPage(topicPage);
    }
    /* other */
    public void increaseTopicPV(int topicID){
        Topic topic = dao.findFirst("select pv from bbs_topic where id=?", topicID);
        if (topic != null){
    ...
    public void increaseTopicPostCount(int topicID){
        int postCount = dao.findFirst("select postCount from bbs_topic where id=?", topicID).getInt("postCount");
    ...
    public void removeAllPageCache() {
        ...
        CacheKit.removeAll(TOPIC_PAGE_FOR_MY_CACHE);
    }
