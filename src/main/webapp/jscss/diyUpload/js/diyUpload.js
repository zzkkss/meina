...
(function( $ ) {
    ...
    $.fn.extend({
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
			...
			if( opt.successall ) {
				var successallCallBack = opt.successall;
				delete opt.successall;
			}
			if( opt.error ) {
			...
			webUploader.on('fileQueued', function( file ) {
				createBox( $fileInput, file ,webUploader,ercishangchuan);
				ercishangchuan=false;
			});
			...
			var path='';
			var ercishangchuan=false;//处理二次上传
			//全部上传结束后触发;
			webUploader.on('uploadFinished', function(){
				ercishangchuan=true;
				$fileInput.next('.parentFileBox').children('.diyButton').remove();
				if ( successallCallBack ) {
					successallCallBack( path );
				}
				path='';
			});
			//绑定发送至服务端返回后触发事件;
	...
	//创建文件操作div;	
	function createBox( $fileInput, file, webUploader,ercishangchuan ) {
...
//					removeLi( $('#fileBox_'+fileArr[i].id), fileArr[i].id, webUploader );
//				}
			$('.parentFileBox li').each(function(){
				$(this).remove();
			});
		}
		//添加父系容器;
